...
//  minor extension of AKAudioPlayer.swift
//  AudioKit
//
//  due to the fileprivate nature of AKAudioPlayer's
//  'internalPlayer' and 'playing' variables, a simple extension of
//  AKAudioPlayer would not be sufficient to implement
//  the desired functionality. I tried every method I could think of
//  to work around these private variables, but in the end, modifying
//  AKAudioPlayer.swift was the only way I could make it work.
//
...
/*
** changes made to AKAudioPlayer.swift **
Line 360: ** added ** func play()
Line 443: ** edited ** func internalCompletionHandler()
*/
open class CustomAudioPlayer: AKNode, AKToggleable {
    ...
    open func start() {
        ...
        if !playing {
            if audioFileBuffer != nil {
                ...
                scheduleBuffer( secondsToAVAudioTime(0) )
                print("AKAudioPlayer.start()")
                playing = true
                paused = false
                internalPlayer.play()
    ...
    /*************** edited *************** 
    Modified play function. Allows for immediate playing
    upon button presses.
    */
    open func play() {
        if(playing) {
    ...
    Default action was to stop audio file.
    Changed such that no default completion action occurs. 
    */
